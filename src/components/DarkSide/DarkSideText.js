...
import evilsQuotes from '../../assets/Quotes/EvilQuotes'
import './darksidetext.css'
const DarkSideText = () => {



 const numberGenerator = Math.floor(Math.random() * evilsQuotes.length)
  const quoteGenerator = evilsQuotes[numberGenerator].split(" ").map((el) =>`${el.toString()} `).join("")


{/* <span>Wel</span><span>come</span> <span>to</span> <span>Dark</span> <span>Side</span><span>!</span> */}
